...
*Une architecture micro-services pour l'évaluation automatisée du risque de crédit.*
...
Pour garantir la performance et la scalabilité, le projet s'appuie sur les technologies suivantes :
...
| ⚙️ **Backend API** | `FastAPI` *(Backend haute performance avec documentation Swagger)* |
| 🖥️ **Interface Utilisateur** | `Streamlit` *(Dashboard interactif pour les prédictions temps réel)* |
...
| ☁️ **Cloud Infrastructure** | `Amazon ECR` *(Registre)* & `Amazon ECS Fargate` *(Serverless)* |

---

## 🏗️ Architecture du Projet

Le projet respecte une séparation stricte des responsabilités pour garantir la maintenance :
...
### 🐳 Option : Mode Docker (Recommandé)

Pour lancer l'écosystème complet en local sans installation de dépendances complexes :
...
🎯 **Accès rapides :**
  ...
  * **API Backend :** [http://localhost:8000](https://www.google.com/search?q=http://localhost:8000) *(Swagger : `http://localhost:8000/docs`)*
  * **Interface UI :** [http://localhost:8501](https://www.google.com/search?q=http://localhost:8501)

\<details\>
\<summary\>\<b\>🐍 Option 2 : Installation Manuelle (Cliquez pour dérouler)\</b\>\</summary\>
<br>
...
L'étape d'exploration a permis de mettre en évidence les signaux faibles et forts du risque :
  ...
  * ⚙️ **Modèle :** XGBoost avec stratification pour compenser le déséquilibre des classes (18.5% de défauts).
...
Ibrahima Ba • Mahamat Sultan • Moustapha Mendy
